...
namespace tage {
    class IdxTagInfo {
    public:
        void setIdxAndTag(int32_t idx, uint64_t tag, uint32_t pos) {
            idx_=idx;
            tag_=tag;
            pos_=pos;
        }
    ...
    public:
        ...
        void saveIdxAndTag(int32_t bank, int32_t idx, uint64_t tag, uint32_t pos);
        ...
        uint64_t getSavedPos(int32_t bank) const { return saved_idx_tag_.at(bank).getPos(); }
...
template<typename PredT>
void PredResult<PredT>::saveIdxAndTag(int32_t bank, int32_t idx, uint64_t tag, uint32_t pos)
{
    assert(!saved_idx_tag_.at(bank).isValid());
    saved_idx_tag_.at(bank).setIdxAndTag(idx, tag, pos);
}
