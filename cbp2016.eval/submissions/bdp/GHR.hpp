...
namespace tage {
    ...
    public:
        ...
        GHR(const uint32_t pc_bits_per_grain,
            ...
            total_bits_per_grain_(pc_bits_per_grain_+tgt_bits_per_grain_),
            pc_mask_((0x1ULL << pc_bits_per_grain)-1),
            tgt_mask_((0x1ULL << tgt_bits_per_grain)-1),
            sig_mask_((0x1ULL << hist_bits_per_br)-1)
        {
            //sparta_assert(total_bits_per_grain_ <= sizeof(sig_mask_)*8,
                                   ...
                                   const uint64_t target)
        {
            ...
            while (bits_consumed<total_bits_per_grain_) {
                signature      ^= pc_plus_tgt;
                pc_plus_tgt   >>= hist_bits_per_br_;
                bits_consumed  += hist_bits_per_br_;
            }
            signature &= sig_mask_;
    ...
    private:
        ...
        const uint32_t total_bits_per_grain_; ///< Total num of pc and target bits
